...
class SparkSearchTool(BaseTool):
    ...
    @classmethod
    def from_api_key(cls, api_key: str, api_secret: str, app_id: str, max_results: int = max_results):
        return cls(api_key=api_key, api_secret=api_secret, app_id=app_id, max_results=max_results)
    ...
    def _sync_search(self, query: str) -> str:
        ...
        url = self._build_auth_url("https://cbm-search-api.cn-huabei-1.xf-yun.com/biz/search")
        json_body = {
            "appId": self.app_id,
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Spark搜索API错误: {response.status_code}, {response_text}")
    ...
    def _build_auth_url(self, request_url: str, method: str = "POST") -> str:
        ...
        authorization_origin = (
            f'api_key="{self.api_key}", algorithm="hmac-sha256", headers="host date request-line", signature="{signature_base64}"'
        )
